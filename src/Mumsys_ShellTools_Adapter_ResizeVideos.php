...
/**
 * Mumsys_ShellTools_Adapter_ResizeVideos
...
/**
 * Resize Videos incl. keep ratio using 'ffmpeg'
 */
class Mumsys_ShellTools_Adapter_ResizeVideos
    extends Mumsys_ShellTools_Adapter_Abstract
{
     ...
     *
     * @var array{
     *  resizevideos: array{size:string, suffix:string}
     * }
     */
...
//exit(1);
     ...
     *
     * @param array{resizevideos: array{
     *  source:string, size:string, suffix:string, target:string
    ...
    private function _prepareCommands(): array
    {
        ...
        foreach ( $fileList as $filename ) {
            ...
            $cmd = sprintf(
                ...
                '%1$s -i "%2$s" -filter:v scale=%3$s:-1 -c:a copy "%4$s"',
                $cmdBase,
                $workingDir . '/' . $filename,
                $this->_results['size'],
                $target
